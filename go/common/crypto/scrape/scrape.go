...
// Reveal executes the commit phase of the protocol, generating a reveal
// message to be broadcasted to all participants.
...
// composite entropy and the indeses of the participants that contributed fully.
...
func NewKeyPair() (*Scalar, *Point, error) {
	scalar, err := newKyberScalar()
	if err != nil {
		return nil, nil, err
	}
	point := suite.Point().Mul(scalar, nil)

	return &Scalar{inner: scalar}, &Point{inner: point}, nil
}

func newKyberScalar() (s kyber.Scalar, err error) {
	// Not sure this can ever really happen.
	defer func() {
		if r := recover(); r != nil {
			err = fmt.Errorf("%+v", r)
			err = fmt.Errorf("scrape: failed to generate scalar: %w", err)
		}
	}()
...
type CommitState struct {
	...
	DecryptedShare *PubVerShare `json:"decrypted_share"`
}

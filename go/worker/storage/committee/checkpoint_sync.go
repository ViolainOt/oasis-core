...
func (n *Node) nodeWorker(
	...
	for {
		...
		default:
			n.logger.Error("can't fetch chunk from storage node", "node", conn.Node.ID, "chunk", chunk.Index)
			return err
		}
		...
		case result.done:
			...
			return nil
		case result.err != nil:
			n.logger.Error("chunk restoration failed", "node", conn.Node.ID, "chunk", chunk.Index, "err", result.err)
			fallthrough
...
func (n *Node) handleCheckpoint(check *checkpoint.Metadata, committeeClient committee.Client, groupSize uint64) (int, error) {
	...
	if err := n.localStorage.Checkpointer().StartRestore(n.ctx, check); err != nil {
		return checkpointStatusNext, fmt.Errorf("can't start checkpoint restore: %w", err)
	}

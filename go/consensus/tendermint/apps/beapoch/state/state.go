...
// ConsensusParameters is the combined beapoch consensus parameters.
type ConsensusParameters struct {
	EpochTime epochtime.ConsensusParameters `json:"epochtime"`
...
func (s *ImmutableState) GetEpoch(ctx context.Context) (epochtime.EpochTime, int64, error) {
	data, err := s.is.Get(ctx, epochCurrentKeyFmt.Encode())
	if err != nil {
		return epochtime.EpochInvalid, 0, abciAPI.UnavailableStateError(err)
	}
	if data == nil {
		return epochtime.EpochTime(0), 0, nil
	}
...
func (s *ImmutableState) ConsensusParameters(ctx context.Context) (*ConsensusParameters, error) {
	data, err := s.is.Get(ctx, parametersKeyFmt.Encode())
	if err != nil {
		return nil, abciAPI.UnavailableStateError(err)
	}
	if data == nil {
		return nil, errors.New("tendermint/beapochtime: expected consensus parameters to be present in app state")
	}

	var params ConsensusParameters
...
func (s *MutableState) SetBeacon(ctx context.Context, newBeacon []byte) error {
	if l := len(newBeacon); l != beacon.BeaconSize {
		return fmt.Errorf("tendermint/beapochtime: unexpected beacon size: %d", l)
	}
...
func (s *MutableState) SetEpoch(ctx context.Context, epoch epochtime.EpochTime, height int64) error {
	state := EpochTimeState{Epoch: epoch, Height: height}
	err := s.ms.Insert(ctx, epochCurrentKeyFmt.Encode(), cbor.Marshal(state))
	return abciAPI.UnavailableStateError(err)
}

func (s *MutableState) SetFutureEpoch(ctx context.Context, epoch epochtime.EpochTime, height int64) error {
	future, err := s.GetFutureEpoch(ctx)
	if err != nil {
		return err
	}
	if future != nil {
		return fmt.Errorf("tendermint/beapochtime: future epoch already pending")
	}
...
func (s *MutableState) SetConsensusParameters(ctx context.Context, params *ConsensusParameters) error {
